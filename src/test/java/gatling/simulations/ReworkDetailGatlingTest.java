...
/**
 * Performance test for the ReworkDetail entity.
 */
public class ReworkDetailGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reworkDetails").get("/api/rework-details").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reworkDetail")
                        .post("/api/rework-details")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"uicode\": \"SAMPLE_TEXT\"" +
                                ", \"pdnDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"reworkDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"numberOfCTNs\": 0" +
                                ", \"startCTNNumber\": 0" +
                                ", \"endCTNNumber\": 0" +
                                ", \"status\": \"PENDING\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reworkDetail_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created reworkDetail").get("${new_reworkDetail_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created reworkDetail").delete("${new_reworkDetail_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReworkDetail entity").exec(scn);
