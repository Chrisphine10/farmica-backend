...
/**
 * Performance test for the SalesDetail entity.
 */
public class SalesDetailGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all salesDetails").get("/api/sales-details").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new salesDetail")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"uicode\": \"SAMPLE_TEXT\"" +
                                ", \"salesDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"numberOfCTNs\": 0" +
                                ", \"receivedCTNs\": 0" +
                                ", \"startCTNNumber\": 0" +
                                ", \"endCTNNumber\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_salesDetail_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created salesDetail").get("${new_salesDetail_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created salesDetail").delete("${new_salesDetail_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SalesDetail entity").exec(scn);
