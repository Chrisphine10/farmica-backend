...
/**
 * Performance test for the StyleReport entity.
 */
public class StyleReportGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all styleReports").get("/api/style-reports").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new styleReport")
                        .post("/api/style-reports")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"createdAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"totalStyleInWarehouse\": 0" +
                                ", \"totalStyleInSales\": 0" +
                                ", \"totalStyleInRework\": 0" +
                                ", \"totalStyleInPacking\": 0" +
                                ", \"totalStyle\": 0" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_styleReport_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created styleReport").get("${new_styleReport_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created styleReport").delete("${new_styleReport_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the StyleReport entity").exec(scn);
