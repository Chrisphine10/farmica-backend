...
/**
 * Performance test for the FarmicaReport entity.
 */
public class FarmicaReportGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all farmicaReports").get("/api/farmica-reports").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new farmicaReport")
                        .post("/api/farmica-reports")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"totalItemsInSales\": 0" +
                                ", \"totalItemsInRework\": 0" +
                                ", \"totalItemsInPacking\": 0" +
                                ", \"totalItems\": 0" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_farmicaReport_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created farmicaReport").get("${new_farmicaReport_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created farmicaReport").delete("${new_farmicaReport_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FarmicaReport entity").exec(scn);
