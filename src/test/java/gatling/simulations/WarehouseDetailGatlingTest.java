...
/**
 * Performance test for the WarehouseDetail entity.
 */
public class WarehouseDetailGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all warehouseDetails").get("/api/warehouse-details").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new warehouseDetail")
                        .post("/api/warehouse-details")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"uicode\": \"SAMPLE_TEXT\"" +
                                ", \"warehouseDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"numberOfCTNs\": 0" +
                                ", \"receivedCTNs\": 0" +
                                ", \"startCTNNumber\": 0" +
                                ", \"endCTNNumber\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_warehouseDetail_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created warehouseDetail").get("${new_warehouseDetail_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created warehouseDetail").delete("${new_warehouseDetail_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the WarehouseDetail entity").exec(scn);
