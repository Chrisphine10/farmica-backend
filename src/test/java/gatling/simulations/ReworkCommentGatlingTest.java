...
/**
 * Performance test for the ReworkComment entity.
 */
public class ReworkCommentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reworkComments").get("/api/rework-comments").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reworkComment")
                        .post("/api/rework-comments")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"comment\": null" + ", \"createdAt\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reworkComment_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created reworkComment").get("${new_reworkComment_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created reworkComment").delete("${new_reworkComment_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReworkComment entity").exec(scn);
