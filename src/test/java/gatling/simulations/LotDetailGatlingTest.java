...
/**
 * Performance test for the LotDetail entity.
 */
public class LotDetailGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all lotDetails").get("/api/lot-details").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new lotDetail")
                        .post("/api/lot-details")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"lotNo\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_lotDetail_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created lotDetail").get("${new_lotDetail_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created lotDetail").delete("${new_lotDetail_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the LotDetail entity").exec(scn);
