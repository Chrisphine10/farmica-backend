...
/**
 * Performance test for the PackingZoneDetail entity.
 */
public class PackingZoneDetailGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all packingZoneDetails")
                    .get("/api/packing-zone-details")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new packingZoneDetail")
                        .post("/api/packing-zone-details")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"packageDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"weightReceived\": 0" +
                                ", \"weightBalance\": 0" +
                                ", \"numberOfCTNs\": 0" +
                                ", \"receivedCTNs\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_packingZoneDetail_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created packingZoneDetail").get("${new_packingZoneDetail_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created packingZoneDetail").delete("${new_packingZoneDetail_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PackingZoneDetail entity").exec(scn);
