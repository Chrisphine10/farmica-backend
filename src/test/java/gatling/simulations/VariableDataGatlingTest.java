...
/**
 * Performance test for the VariableData entity.
 */
public class VariableDataGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all variableData").get("/api/variable-data").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new variableData")
                        .post("/api/variable-data")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"accumulation\": 0" + ", \"aiAccessCode\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_variableData_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created variableData").get("${new_variableData_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created variableData").delete("${new_variableData_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the VariableData entity").exec(scn);
