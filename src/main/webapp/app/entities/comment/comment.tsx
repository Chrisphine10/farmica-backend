  ...
  return (
    <div>
      <h2 id="comment-heading" data-cy="CommentHeading">
        <Translate contentKey="farmicaApp.comment.home.title">Comments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="farmicaApp.comment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/comment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="farmicaApp.comment.home.createLabel">Create new Comment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {commentList && commentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="farmicaApp.comment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('zoneId')}>
                  <Translate contentKey="farmicaApp.comment.zoneId">Zone Id</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('zoneId')} />
                </th>
                ...
                <th>
                  <Translate contentKey="farmicaApp.comment.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {commentList.map((comment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`farmicaApp.CurrentZone.${comment.status}`} />
                  </td>
                  <td>{comment.zoneId}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="farmicaApp.comment.home.notFound">No Comments found</Translate>
            </div>
          )
        )}
      </div>
